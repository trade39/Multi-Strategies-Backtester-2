...
# --- Early Imports for Settings & Page Config ---
from config import settings # Needed for APP_TITLE
...
db_init_error = None
try:
    database_manager.init_db() # This function now uses @st.cache_resource for its connection
    logger.info("Database initialization sequence complete from app.py.")
except Exception as e:
    db_init_error = e
    logger.error(f"Failed to initialize database from app.py: {e}", exc_info=True)
    # Error will be displayed after page config

# --- Display Database Error if any ---
if db_init_error:
    st.error(f"CRITICAL DATABASE ERROR: Could not initialize the application database. "
             f"Please check logs. Some features may be unavailable. Error: {db_init_error}")
    # Optionally, you might want to st.stop() here if the DB is absolutely critical
    # st.stop()

# --- Initialize AI Service in session state ---
if 'ai_service' not in st.session_state:
    try:
        st.session_state.ai_service = AIModelService()
        logger.info("AIModelService initialized and stored in session state.")
    except Exception as e:
        logger.error(f"Failed to initialize AIModelService: {e}", exc_info=True)
        st.error(f"AI Service Error: Could not initialize the AI Model Service. AI features may be unavailable. Error: {e}")
...
def load_custom_css(css_file_path):
    try:
        ...
        logger.info(f"Custom CSS '{css_file_path}' loaded.")
    except FileNotFoundError:
        logger.warning(f"CSS file not found: {css_file_path}. Using default styles.")
    except Exception as e:
...
def initialize_app_session_state():
    defaults = {
        ...
        'enable_ai_signal_filtering': False,
        'selected_ai_filter_model': settings.DEFAULT_AI_SIGNAL_FILTER_MODEL if hasattr(settings, 'DEFAULT_AI_SIGNAL_FILTER_MODEL') else "RandomForest",
        'enable_ai_regime_detection': False,
        'current_market_regime': "Unknown",
    }
    for key, value in defaults.items():
        if key not in st.session_state:
            st.session_state[key] = value
    logger.debug("Application session state initialized with defaults.")

if not db_init_error:
    initialize_app_session_state()
else:
    logger.warning("Skipping full session state initialization due to DB error.")
    if 'run_analysis_clicked_count' not in st.session_state:
         st.session_state.run_analysis_clicked_count = 0


# --- Main UI Block (conditional on DB initialization) ---
if not db_init_error:
    # --- Sidebar Inputs ---
    ...
    st.session_state.selected_strategy = st.sidebar.selectbox(
        ...
        index=settings.AVAILABLE_STRATEGIES.index(st.session_state.get('selected_strategy', settings.DEFAULT_STRATEGY)),
        key="strategy_selector_v3" # Incremented key
    )
    ...
    selected_ticker_name = st.sidebar.selectbox(
        ...
        index=0,
        key="ticker_sel_v13" # Incremented key
    )
    ticker_symbol = settings.DEFAULT_TICKERS[selected_ticker_name]

    current_tf_value_in_state = st.session_state.get('selected_timeframe_value', settings.DEFAULT_STRATEGY_TIMEFRAME)
    ...
    selected_timeframe_display = st.sidebar.selectbox(
        ...
        key="timeframe_selector_ui_main_v13" # Incremented key
    )
    ...
    if max_history_limit_days:
        default_start_days_ago = min(15, max_history_limit_days - 1 if max_history_limit_days > 1 else 0)
    elif ui_current_interval == "1wk":
        default_start_days_ago = 30 * 7
    else:
    ...
    start_date_ui = st.sidebar.date_input(
        ...
        max_value=max_possible_start_date, key=f"start_date_widget_{ui_current_interval}_v13", help=f"Start date. {date_input_help_suffix}"
    )
    ...
    end_date_ui = st.sidebar.date_input(
        ...
        max_value=today, key=f"end_date_widget_{ui_current_interval}_v13", help=f"End date. {date_input_help_suffix}"
    )

    initial_capital_ui = st.sidebar.number_input("Initial Capital ($):", min_value=1.0, value=float(settings.DEFAULT_INITIAL_CAPITAL), step=1000.0, format="%.2f")
    risk_per_trade_percent_ui = st.sidebar.number_input("Risk per Trade (%):", min_value=0.01, max_value=100.0, value=float(settings.DEFAULT_RISK_PER_TRADE_PERCENT), step=0.1, format="%.1f")

    st.sidebar.subheader("Common Strategy Parameters")
    sl_points_single_ui = st.sidebar.number_input("SL (points):", min_value=0.01, value=float(settings.DEFAULT_STOP_LOSS_POINTS), step=0.1, format="%.2f", key="sl_s_man_v13")
    rrr_single_ui = st.sidebar.number_input("RRR:", min_value=0.1, value=float(settings.DEFAULT_RRR), step=0.1, format="%.1f", key="rrr_s_man_v13")
    ...
    if selected_strategy_name == "Gap Guardian":
        ...
        c1,c2=st.sidebar.columns(2); entry_start_hour_single_ui = c1.number_input("Start Hr",0,23,settings.DEFAULT_ENTRY_WINDOW_START_HOUR,1,key="esh_s_man_v13")
        entry_start_minute_single_ui = c2.number_input("Start Min",0,59,settings.DEFAULT_ENTRY_WINDOW_START_MINUTE,15,key="esm_s_man_v13")
        c1,c2=st.sidebar.columns(2); entry_end_hour_single_ui = c1.number_input("End Hr",0,23,settings.DEFAULT_ENTRY_WINDOW_END_HOUR,1,key="eeh_s_man_v13")
        entry_end_minute_single_ui = c2.number_input("End Min",0,59,settings.DEFAULT_ENTRY_WINDOW_END_MINUTE,15,key="eem_s_man_v13", help="Usually 00.")
    ...
    ai_service_available = st.session_state.get('ai_service') is not None
    default_ai_model = settings.DEFAULT_AI_SIGNAL_FILTER_MODEL if hasattr(settings, 'DEFAULT_AI_SIGNAL_FILTER_MODEL') else "RandomForest"
    available_ai_models = settings.AVAILABLE_AI_FILTER_MODELS if hasattr(settings, 'AVAILABLE_AI_FILTER_MODELS') else ["RandomForest", "XGBoost"]


    st.session_state.enable_ai_signal_filtering = st.sidebar.checkbox(
        "Enable AI Signal Filtering", value=st.session_state.get('enable_ai_signal_filtering', False), 
        key="enable_ai_filter_v1_fixed", disabled=not ai_service_available
    )
    if st.session_state.enable_ai_signal_filtering and ai_service_available:
        selected_model_val = st.session_state.get('selected_ai_filter_model', default_ai_model)
        model_idx = available_ai_models.index(selected_model_val) if selected_model_val in available_ai_models else 0
        
        st.session_state.selected_ai_filter_model = st.sidebar.selectbox(
            "Signal Filter Model:", options=available_ai_models, index=model_idx, key="ai_filter_model_sel_v1_fixed"
        )
        if st.session_state.ai_service and st.session_state.ai_service.signal_filter_model is None:
            st.sidebar.caption("⚠️ Filter model not trained. Filtering will be skipped.")
        elif st.session_state.ai_service:
            st.sidebar.caption(f"✅ {st.session_state.selected_ai_filter_model} filter model active.")
    elif not ai_service_available and st.session_state.get('enable_ai_signal_filtering', False):
         st.sidebar.caption("AI Service unavailable. Filtering disabled.")

    st.session_state.enable_ai_regime_detection = st.sidebar.checkbox(
        "Enable AI Regime Detection", value=st.session_state.get('enable_ai_regime_detection', False),
        key="enable_ai_regime_v1_fixed", help="Allows strategies to adapt to market regimes. Feature under development.",
        disabled=not ai_service_available
    )
    if st.session_state.enable_ai_regime_detection and ai_service_available:
        if st.session_state.ai_service and st.session_state.ai_service.regime_detection_model is None:
             st.sidebar.caption("⚠️ Regime model not trained. Detection may be inaccurate.")
        elif st.session_state.ai_service:
            st.sidebar.caption(f"✅ Regime detection model active. Current: {st.session_state.get('current_market_regime', 'Unknown')}")
    elif not ai_service_available and st.session_state.get('enable_ai_regime_detection', False):
        st.sidebar.caption("AI Service unavailable. Regime detection disabled.")

    st.sidebar.subheader("Analysis Configuration")
    analysis_mode_ui = st.sidebar.radio(
        "Analysis Type:", ("Single Backtest", "Parameter Optimization", "Walk-Forward Optimization"),
        index=0, key="analysis_mode_v13" # Incremented key
    )
    ...
    if analysis_mode_ui != "Single Backtest":
        ...
        opt_algo_ui = st.sidebar.selectbox("Algorithm:", settings.OPTIMIZATION_ALGORITHMS, settings.OPTIMIZATION_ALGORITHMS.index(opt_algo_ui), key="opt_algo_v13")
        opt_metric_ui = st.sidebar.selectbox("Optimize Metric:", settings.OPTIMIZATION_METRICS, settings.OPTIMIZATION_METRICS.index(opt_metric_ui), key="opt_metric_v13")
        
        st.sidebar.markdown("**SL Range:**"); c1,c2,c3=st.sidebar.columns(3); sl_min_opt_ui=c1.number_input("Min",value=sl_min_opt_ui,step=0.1,format="%.1f",key="slmin_o13")
        sl_max_opt_ui=c2.number_input("Max",value=sl_max_opt_ui,step=0.1,format="%.1f",key="slmax_o13")
        if opt_algo_ui=="Grid Search": sl_steps_opt_ui=c3.number_input("Steps",2,10,int(sl_steps_opt_ui),1,key="slsteps_o13")
        
        st.sidebar.markdown("**RRR Range:**"); c1,c2,c3=st.sidebar.columns(3); rrr_min_opt_ui=c1.number_input("Min",value=rrr_min_opt_ui,step=0.1,format="%.1f",key="rrrmin_o13")
        rrr_max_opt_ui=c2.number_input("Max",value=rrr_max_opt_ui,step=0.1,format="%.1f",key="rrrmax_o13")
        if opt_algo_ui=="Grid Search": rrr_steps_opt_ui=c3.number_input("Steps",2,10,int(rrr_steps_opt_ui),1,key="rrrsteps_o13")

        if selected_strategy_name == "Gap Guardian":
            st.sidebar.markdown("**Entry Start Hr Range:**"); c1,c2,c3=st.sidebar.columns(3); esh_min_opt_ui=c1.number_input("Min Hr",value=esh_min_opt_ui,min_value=0,max_value=23,step=1,key="eshmin_o13")
            esh_max_opt_ui=c2.number_input("Max Hr",value=esh_max_opt_ui,min_value=0,max_value=23,step=1,key="eshmax_o13")
            if opt_algo_ui=="Grid Search": esh_steps_opt_ui=c3.number_input("Hr Steps",2,5,int(esh_steps_opt_ui),1,key="eshsteps_o13")
            esm_vals_opt_ui=st.sidebar.multiselect("Entry Start Min(s):", [0,15,30,45,50], default=esm_vals_opt_ui, key="esmvals_o13")
            if not esm_vals_opt_ui: esm_vals_opt_ui = [settings.DEFAULT_ENTRY_WINDOW_START_MINUTE]
            st.sidebar.markdown("**Entry End Hr Range:**"); c1,c2,c3=st.sidebar.columns(3); eeh_min_opt_ui=c1.number_input("Min Hr",value=eeh_min_opt_ui,min_value=0,max_value=23,step=1,key="eehmin_o13")
            eeh_max_opt_ui=c2.number_input("Max Hr",value=eeh_max_opt_ui,min_value=0,max_value=23,step=1,key="eehmax_o13")
            if opt_algo_ui=="Grid Search": eeh_steps_opt_ui=c3.number_input("Hr Steps",2,5,int(eeh_steps_opt_ui),1,key="eehsteps_o13")
        
        if opt_algo_ui=="Random Search": rand_iters_ui=st.sidebar.number_input("Random Iterations:",10,500,rand_iters_ui,10,key="randiter_o13")
    ...
    if analysis_mode_ui == "Walk-Forward Optimization":
        ...
        wfo_isd_ui = st.sidebar.number_input("In-Sample (Days):", min_value=MIN_WFO_IS_DAYS, value=st.session_state.get('wfo_isd_ui_val', settings.DEFAULT_WFO_IN_SAMPLE_DAYS), step=10, key="wfoisd_v13")
        wfo_oosd_ui = st.sidebar.number_input("Out-of-Sample (Days):", min_value=MIN_WFO_OOS_DAYS, value=st.session_state.get('wfo_oosd_ui_val', settings.DEFAULT_WFO_OUT_OF_SAMPLE_DAYS), step=5, key="wfoosd_v13")
        wfo_sd_ui = st.sidebar.number_input("Step (Days):", min_value=wfo_oosd_ui, value=st.session_state.get('wfo_sd_ui_val', settings.DEFAULT_WFO_STEP_DAYS), step=5, key="wfosd_v13")
    ...
    if st.sidebar.button("Run Analysis", type="primary", use_container_width=True, key="run_main_v13"): # Incremented key
        ...
        if start_date_ui >= end_date_ui: st.error(f"Error: Start date ({start_date_ui}) must be before end date ({end_date_ui}).")
            ...
            if price_data_df.empty: st.warning(f"No price data found for {selected_ticker_name} ({interval_for_this_run}) for the period {start_date_ui} to {end_date_ui}. Cannot proceed.")
                ...
                current_ai_service = st.session_state.get('ai_service')
                if (st.session_state.enable_ai_signal_filtering or st.session_state.enable_ai_regime_detection) and not current_ai_service:
                    st.error("AI features are enabled, but the AI Service is not available. AI features will be skipped for this run.")
                ...
                strategy_params_for_engine = {
                    ...
                    'enable_signal_filtering': st.session_state.enable_ai_signal_filtering if current_ai_service else False,
                    'enable_regime_detection': st.session_state.enable_ai_regime_detection if current_ai_service else False,
                }
                ...
                if selected_strategy_name == "Gap Guardian":
                    ...
                    strategy_params_for_engine.update({'entry_start_time': entry_start_t, 'entry_end_time': entry_end_t})
                    current_run_parameters_for_db.update({'EntryStartTime': entry_start_t.strftime('%H:%M'), 'EntryEndTime': entry_end_t.strftime('%H:%M')})
                
                if strategy_params_for_engine['enable_signal_filtering']:
                    current_run_parameters_for_db['AI_Filter_Model'] = st.session_state.selected_ai_filter_model
                if strategy_params_for_engine['enable_regime_detection']:
                ...
                if analysis_mode_ui == "Single Backtest":
                    ...
                    with st.spinner("Generating signals & running backtest..."):
                        ...
                        if selected_strategy_name == "Gap Guardian": param_display_str += f", Entry: {strategy_params_for_engine['entry_start_time']:%H:%M}-{strategy_params_for_engine['entry_end_time']:%H:%M}"
                        if strategy_params_for_engine['enable_signal_filtering']: param_display_str += f" (AI Filter: {st.session_state.selected_ai_filter_model})"
                        if strategy_params_for_engine['enable_regime_detection']: param_display_str += " (AI Regime On)"

                        st.session_state.backtest_results = {
                            "trades":trades, "equity_curve":equity, "performance":perf,
                            "params":{"Strategy": selected_strategy_name, "SL": sl_points_single_ui, "RRR": rrr_single_ui, "TF":interval_for_this_run, "EntryDisplay":param_display_str, "src":"Manual"}
                        }
                        ...
                        if perf and not trades.empty:
                            try:
                                database_manager.save_backtest_results(
                                    ...
                                    start_date_dt=start_date_ui, end_date_dt=end_date_ui, initial_capital=initial_capital_ui, risk_per_trade_percent=risk_per_trade_percent_ui,
                                    parameters=current_run_parameters_for_db, source="Manual", performance_metrics=perf, trades_df=trades, equity_curve_series=equity
                                )
                            except Exception as db_save_err:
                                logger.error(f"Failed to save single backtest results to DB: {db_save_err}", exc_info=True)
                                st.warning(f"Could not save backtest results to database. Error: {db_save_err}")
                    ...
                    optimizer_control_params = {
                        'metric_to_optimize': opt_metric_ui, 'strategy_name': selected_strategy_name,
                        'ai_service': current_ai_service,
                        'enable_signal_filtering': st.session_state.enable_ai_signal_filtering if current_ai_service else False,
                        'enable_regime_detection': st.session_state.enable_ai_regime_detection if current_ai_service else False,
                    }
                    ...
                    if analysis_mode_ui == "Parameter Optimization":
                        ...
                        with st.spinner(f"Running {opt_algo_ui}..."):
                            ...
                            if not opt_df.empty:
                                ...
                                db_opt_params_extra = {
                                    'AI_Filter_Enabled': optimizer_control_params['enable_signal_filtering'],
                                    'AI_Filter_Model': st.session_state.selected_ai_filter_model if optimizer_control_params['enable_signal_filtering'] else 'N/A',
                                    'AI_Regime_Enabled': optimizer_control_params['enable_regime_detection']
                                }
                                try:
                                    database_manager.save_optimization_results(
                                        strategy_name=selected_strategy_name, ticker=selected_ticker_name, timeframe=interval_for_this_run,
                                        start_date_dt=start_date_ui, end_date_dt=end_date_ui, optimization_algorithm=opt_algo_ui, optimized_metric=opt_metric_ui,
                                        results_df=opt_df, extra_config=db_opt_params_extra
                                    )
                                except Exception as db_save_err:
                                    logger.error(f"Failed to save optimization results to DB: {db_save_err}", exc_info=True)
                                    st.warning(f"Could not save optimization results to database. Error: {db_save_err}")
                                ...
                                if not valid_opt.empty:
                                    ...
                                    best_params_for_bt_opt_dict = {
                                        'strategy_name': selected_strategy_name, 'stop_loss_points': best_r["SL Points"], 'rrr': best_r["RRR"],
                                        'ai_service': current_ai_service,
                                        'enable_signal_filtering': optimizer_control_params['enable_signal_filtering'],
                                        'enable_regime_detection': optimizer_control_params['enable_regime_detection'],
                                    }
                                    ...
                                    if selected_strategy_name == "Gap Guardian":
                                        ...
                                        best_ee_t = dt_time(int(best_r.get("EntryEndHour",0)),int(best_r.get("EntryEndMinute",0)))
                                        best_params_for_bt_opt_dict['entry_start_time'] = best_es_t; best_params_for_bt_opt_dict['entry_end_time'] = best_ee_t
                                        entry_display_opt += f", Entry: {best_es_t:%H:%M}-{best_ee_t:%H:%M}"
                                        current_run_parameters_for_db_opt['EntryStartTime'] = best_es_t.strftime('%H:%M'); current_run_parameters_for_db_opt['EntryEndTime'] = best_ee_t.strftime('%H:%M')
                                    if optimizer_control_params['enable_signal_filtering']: entry_display_opt += f" (AI Filter: {st.session_state.selected_ai_filter_model})"
                                    if optimizer_control_params['enable_regime_detection']: entry_display_opt += " (AI Regime On)"
                                    current_run_parameters_for_db_opt.update(db_opt_params_extra)
                                        ...
                                        if perf_b and not trades_b.empty:
                                            try:
                                                database_manager.save_backtest_results(
                                                    strategy_name=selected_strategy_name, ticker=selected_ticker_name, timeframe=interval_for_this_run,
                                                    start_date_dt=start_date_ui, end_date_dt=end_date_ui, initial_capital=initial_capital_ui, risk_per_trade_percent=risk_per_trade_percent_ui,
                                                    parameters=current_run_parameters_for_db_opt, source=f"Opt ({opt_algo_ui})", performance_metrics=perf_b, trades_df=trades_b, equity_curve_series=equity_b
                                                )
                                            except Exception as db_save_err:
                                                logger.error(f"Failed to save optimized backtest results to DB: {db_save_err}", exc_info=True)
                                                st.warning(f"Could not save optimized backtest results to database. Error: {db_save_err}")
                        ...
                        wfo_optimizer_config_and_control = {**optimizer_control_params, **actual_params_to_optimize_config}
                        ...
                        with st.spinner(f"Running WFO with {opt_algo_ui}... This will take considerable time."):
                            wfo_log,oos_trades,oos_equity,oos_perf = optimizer.run_walk_forward_optimization(
                                price_data_df,initial_capital_ui,risk_per_trade_percent_ui, wfo_p, opt_algo_ui, 
                                wfo_optimizer_config_and_control, interval_for_this_run, lambda p,s: opt_cb(p,s)
                            )
                            ...
                            if optimizer_control_params['enable_signal_filtering']: wfo_param_display += f" (AI Filter: {st.session_state.selected_ai_filter_model})"
                            if optimizer_control_params['enable_regime_detection']: wfo_param_display += " (AI Regime On)"
                            ...
                            if oos_perf and not oos_trades.empty:
                                try:
                                    wfo_params_for_db = {
                                        "in_sample_days": wfo_p['in_sample_days'], "out_of_sample_days": wfo_p['out_of_sample_days'], "step_days": wfo_p['step_days'], 
                                        "opt_algo": opt_algo_ui, "opt_metric": opt_metric_ui,
                                        'AI_Filter_Enabled': optimizer_control_params['enable_signal_filtering'],
                                        'AI_Filter_Model': st.session_state.selected_ai_filter_model if optimizer_control_params['enable_signal_filtering'] else 'N/A',
                                        'AI_Regime_Enabled': optimizer_control_params['enable_regime_detection']
                                    }
                                    database_manager.save_backtest_results(
                                        strategy_name=selected_strategy_name, ticker=selected_ticker_name, timeframe=interval_for_this_run,
                                        start_date_dt=start_date_ui, end_date_dt=end_date_ui, initial_capital=initial_capital_ui, risk_per_trade_percent=risk_per_trade_percent_ui,
                                        parameters=wfo_params_for_db, source="WFO Aggregated", performance_metrics=oos_perf, trades_df=oos_trades, equity_curve_series=oos_equity
                                    )
                                except Exception as db_save_err:
                                    logger.error(f"Failed to save WFO aggregated results to DB: {db_save_err}", exc_info=True)
                                    st.warning(f"Could not save WFO results to database. Error: {db_save_err}")
                            prog_bar_container.progress(1.0, text="WFO Complete!")
                if prog_bar_container is not None: prog_bar_container.empty() 

    if not db_init_error and ai_service_available:
        st.sidebar.markdown("---")
        st.sidebar.subheader("🧠 AI Model Management")
        if st.sidebar.button("Train AI Signal Filter (Placeholder)", key="train_sig_filter_v1_fixed", disabled=not st.session_state.get('ai_service')):
            # ... (AI training logic as before) ...
            if not st.session_state.price_data.empty and st.session_state.ai_service:
                st.info("Attempting placeholder training for AI Signal Filter...")
                with st.spinner("Training signal filter model... (placeholder)"):
                    signals_for_training = st.session_state.signals 
                    ...
                    if not signals_for_training.empty:
                        features_df = st.session_state.ai_service._prepare_features_for_signal_filtering(
                            st.session_state.price_data.copy(), signals_for_training.copy() 
                        )
                        if not features_df.empty:
                            ...
                            if not aligned_features.empty:
                                st.session_state.ai_service.train_signal_filter_model(
                                    aligned_features, aligned_labels, model_type=st.session_state.selected_ai_filter_model
                                )
                                st.success(f"Placeholder AI Signal Filter ({st.session_state.selected_ai_filter_model}) 'trained'!")
                            else: st.error("Could not align features and labels for training.")
                        else: st.error("No features could be generated for training the AI signal filter.")
                    else: st.error("No signals available for AI model training.")
            elif not st.session_state.ai_service: st.warning("AI Service not available for training.")
            else: st.warning("Load price data first to enable AI model training.")


    # --- Corrected Display Area Logic ---
    main_tabs_to_display_names = []
    if st.session_state.get('backtest_results'): main_tabs_to_display_names.append("📊 Backtest Performance")
    opt_results_df_to_check = st.session_state.get('optimization_results_df', pd.DataFrame())
    if not opt_results_df_to_check.empty and analysis_mode_ui == "Parameter Optimization": main_tabs_to_display_names.append("⚙️ Optimization Results (Full Period)")
    if st.session_state.get('wfo_results') and analysis_mode_ui == "Walk-Forward Optimization": main_tabs_to_display_names.append("🚶 Walk-Forward Analysis")

    if main_tabs_to_display_names:
        tabs_key_suffix = f"_{st.session_state.run_analysis_clicked_count}_{selected_strategy_name}_AI{st.session_state.enable_ai_signal_filtering}"
        created_tabs = st.tabs(main_tabs_to_display_names) 
        tab_map = dict(zip(main_tabs_to_display_names, created_tabs))

        if "📊 Backtest Performance" in tab_map and st.session_state.backtest_results:
            with tab_map["📊 Backtest Performance"]:
                # ... (Detailed backtest performance display logic as before) ...
                ...
                param_display_from_run = run_params.get("EntryDisplay", "") 
                if param_display_from_run: details_md_parts.append(f"<li><strong>Parameters:</strong> {param_display_from_run}</li>")
                ...
                detail_tabs_perf = st.tabs(detail_tabs_list) # Unique key for these tabs
                with detail_tabs_perf[0]:
                ...
                if "🔍 Generated Signals (Final)" in detail_tabs_list:
                    ...
                    idx_offset = 1
                with detail_tabs_perf[3+idx_offset]:
                    if not st.session_state.price_data.empty:
                        ...
                        csv_data = st.session_state.price_data.to_csv(index=True).encode('utf-8'); st.download_button("Download Full Price Data CSV", csv_data, f"{ticker_symbol}_price_data.csv", 'text/csv', key='dl_raw_price_main_v13')
                    else: st.info("Raw price data is not available.")
        
        if "⚙️ Optimization Results (Full Period)" in tab_map and not opt_results_df_to_check.empty:
            with tab_map["⚙️ Optimization Results (Full Period)"]:
                # ... (Detailed optimization results display logic as before) ...
                ...
                csv_opt_disp = opt_df_display.to_csv(index=False).encode('utf-8'); st.download_button("Download Optimization CSV", csv_opt_disp, f"{ticker_symbol}_{selected_strategy_name}_opt_results.csv", 'text/csv', key='dl_opt_csv_main_v13')
        ...
        if "🚶 Walk-Forward Analysis" in tab_map and st.session_state.wfo_results:
            with tab_map["🚶 Walk-Forward Analysis"]:
                # ... (Detailed WFO display logic as before) ...
                ...
                csv_wfo_log_disp = wfo_res_disp["log"].to_csv(index=False).encode('utf-8'); st.download_button("Download WFO Log CSV", csv_wfo_log_disp, f"{ticker_symbol}_{selected_strategy_name}_wfo_log.csv", 'text/csv', key='dl_wfo_log_main_v13')
                ...
                if not wfo_res_disp["oos_trades"].empty:
                    ...
                    csv_wfo_trades_disp = wfo_res_disp["oos_trades"].to_csv(index=False).encode('utf-8'); st.download_button("Download WFO OOS Trades CSV", csv_wfo_trades_disp, f"{ticker_symbol}_{selected_strategy_name}_wfo_oos_trades.csv", 'text/csv', key='dl_wfo_trades_main_v13')
                else: st.info("No out-of-sample trades generated during WFO.")
    
    elif st.session_state.get('run_analysis_clicked_count', 0) > 0:
    ...
    else: # This 'else' correctly corresponds to the 'if main_tabs_to_display_names:' and 'elif run_analysis_clicked_count > 0:'
          # It will execute if no tabs are to be shown AND analysis hasn't been clicked yet.
          # This assumes we are already inside the 'if not db_init_error:' block.
        st.info("Configure parameters in the sidebar and click 'Run Analysis'.")

    st.sidebar.markdown("---")
    st.sidebar.info(f"App Version: 0.7.2 | AI Integrated | Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}") # Version increment
...
else: # This 'else' corresponds to the main 'if not db_init_error:'
    st.warning("Application UI cannot be fully displayed due to the critical database error mentioned above.")
