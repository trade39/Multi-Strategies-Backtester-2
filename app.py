...
if db_init_error:
    ...
    # Optionally, you might want to st.stop() here if the DB is absolutely critical for any further operation
    # st.stop()

# --- Initialize AI Service in session state ---
# Accessing st.session_state is now safe after st.set_page_config
if 'ai_service' not in st.session_state:
    try:
        ...
        st.session_state.ai_service = None # Ensure it's None if init fails

# --- Load Custom CSS ---
# This is also a Streamlit command (st.markdown), so it's fine here.
...
load_custom_css("static/style.css") # Assuming 'static/style.css'
...
def initialize_app_session_state():
    defaults = {
        ...
        'selected_ai_filter_model': "RandomForest", # Ensure this matches an available model in AIModelService
...
if not db_init_error: # Only initialize session state if DB is okay, or adjust as needed
    initialize_app_session_state()
else:
    logger.warning("Skipping full session state initialization due to DB error.")
    # Initialize minimal state if needed for error display or basic UI
    if 'run_analysis_clicked_count' not in st.session_state: # Example minimal init
         st.session_state.run_analysis_clicked_count = 0


# --- Sidebar Inputs ---
# Ensure this section and subsequent UI elements are only rendered if critical initializations (like DB) are successful,
# or design them to handle the absence of these services gracefully.
if not db_init_error: # Proceed with UI only if DB is okay
    st.sidebar.header("Backtest Configuration")

    st.session_state.selected_strategy = st.sidebar.selectbox(
        "Select Strategy:",
        options=settings.AVAILABLE_STRATEGIES,
        index=settings.AVAILABLE_STRATEGIES.index(st.session_state.get('selected_strategy', settings.DEFAULT_STRATEGY)), # Use .get for safety
        key="strategy_selector_v2_fixed"
    )
    ...
    selected_ticker_name = st.sidebar.selectbox(
        ...
        index=0, # Or retrieve last selected if stored
        key="ticker_sel_v12_fixed"
    )
    ...
    selected_timeframe_display = st.sidebar.selectbox(
        ...
        key="timeframe_selector_ui_main_v12_fixed"
    )
    ...
    # Default start date logic needs careful handling if max_history_limit_days is small
    ...
    max_possible_start_date = today - timedelta(days=1) # Cannot start on today for historical data
    if default_start_date_value > max_possible_start_date: default_start_date_value = max_possible_start_date
    # Ensure start is still valid if pushed to max_possible_start_date
    if default_start_date_value < min_allowable_start_date_for_ui: default_start_date_value = min_allowable_start_date_for_ui
    ...
    start_date_ui = st.sidebar.date_input(
        ...
        key=f"start_date_widget_{ui_current_interval}_v12_fixed",
    ...
    end_date_ui = st.sidebar.date_input(
        ...
        max_value=today,
        key=f"end_date_widget_{ui_current_interval}_v12_fixed",
    ...
    sl_points_single_ui = st.sidebar.number_input("SL (points):", min_value=0.01, value=float(settings.DEFAULT_STOP_LOSS_POINTS), step=0.1, format="%.2f", key="sl_s_man_v12_fixed")
    rrr_single_ui = st.sidebar.number_input("RRR:", min_value=0.1, value=float(settings.DEFAULT_RRR), step=0.1, format="%.1f", key="rrr_s_man_v12_fixed")
    ...
    if selected_strategy_name == "Gap Guardian":
        ...
        c1,c2=st.sidebar.columns(2); entry_start_hour_single_ui = c1.number_input("Start Hr",0,23,settings.DEFAULT_ENTRY_WINDOW_START_HOUR,1,key="esh_s_man_v12_fixed")
        entry_start_minute_single_ui = c2.number_input("Start Min",0,59,settings.DEFAULT_ENTRY_WINDOW_START_MINUTE,15,key="esm_s_man_v12_fixed")
        c1,c2=st.sidebar.columns(2); entry_end_hour_single_ui = c1.number_input("End Hr",0,23,settings.DEFAULT_ENTRY_WINDOW_END_HOUR,1,key="eeh_s_man_v12_fixed")
        entry_end_minute_single_ui = c2.number_input("End Min",0,59,settings.DEFAULT_ENTRY_WINDOW_END_MINUTE,15,key="eem_s_man_v12_fixed", help="Usually 00.")
    ...
    st.session_state.enable_ai_signal_filtering = st.sidebar.checkbox(
        ...
        value=st.session_state.get('enable_ai_signal_filtering', False), 
        key="enable_ai_filter_v1_fixed",
    ...
    if st.session_state.enable_ai_signal_filtering and ai_service_available:
        st.session_state.selected_ai_filter_model = st.sidebar.selectbox(
            "Signal Filter Model:",
            options=settings.AVAILABLE_AI_FILTER_MODELS, # Assuming this is in settings
            index=settings.AVAILABLE_AI_FILTER_MODELS.index(st.session_state.get('selected_ai_filter_model', settings.DEFAULT_AI_SIGNAL_FILTER_MODEL)),
            key="ai_filter_model_sel_v1_fixed"
        )
    ...
    elif not ai_service_available and st.session_state.enable_ai_signal_filtering:
    ...
    st.session_state.enable_ai_regime_detection = st.sidebar.checkbox(
        ...
        value=st.session_state.get('enable_ai_regime_detection', False),
        key="enable_ai_regime_v1_fixed",
    ...
    elif not ai_service_available and st.session_state.enable_ai_regime_detection:
    ...
    if analysis_mode_ui != "Single Backtest":
        ...
        opt_algo_ui = st.sidebar.selectbox("Algorithm:", settings.OPTIMIZATION_ALGORITHMS, settings.OPTIMIZATION_ALGORITHMS.index(opt_algo_ui), key="opt_algo_v12_fixed")
        opt_metric_ui = st.sidebar.selectbox("Optimize Metric:", settings.OPTIMIZATION_METRICS, settings.OPTIMIZATION_METRICS.index(opt_metric_ui), key="opt_metric_v12_fixed")
        
        st.sidebar.markdown("**SL Range:**"); c1,c2,c3=st.sidebar.columns(3); sl_min_opt_ui=c1.number_input("Min",value=sl_min_opt_ui,step=0.1,format="%.1f",key="slmin_o12_fixed")
        sl_max_opt_ui=c2.number_input("Max",value=sl_max_opt_ui,step=0.1,format="%.1f",key="slmax_o12_fixed")
        if opt_algo_ui=="Grid Search": sl_steps_opt_ui=c3.number_input("Steps",2,10,int(sl_steps_opt_ui),1,key="slsteps_o12_fixed")
        
        st.sidebar.markdown("**RRR Range:**"); c1,c2,c3=st.sidebar.columns(3); rrr_min_opt_ui=c1.number_input("Min",value=rrr_min_opt_ui,step=0.1,format="%.1f",key="rrrmin_o12_fixed")
        rrr_max_opt_ui=c2.number_input("Max",value=rrr_max_opt_ui,step=0.1,format="%.1f",key="rrrmax_o12_fixed")
        if opt_algo_ui=="Grid Search": rrr_steps_opt_ui=c3.number_input("Steps",2,10,int(rrr_steps_opt_ui),1,key="rrrsteps_o12_fixed")

        if selected_strategy_name == "Gap Guardian":
            st.sidebar.markdown("**Entry Start Hr Range:**"); c1,c2,c3=st.sidebar.columns(3); esh_min_opt_ui=c1.number_input("Min Hr",value=esh_min_opt_ui,min_value=0,max_value=23,step=1,key="eshmin_o12_fixed")
            esh_max_opt_ui=c2.number_input("Max Hr",value=esh_max_opt_ui,min_value=0,max_value=23,step=1,key="eshmax_o12_fixed")
            if opt_algo_ui=="Grid Search": esh_steps_opt_ui=c3.number_input("Hr Steps",2,5,int(esh_steps_opt_ui),1,key="eshsteps_o12_fixed")
            esm_vals_opt_ui=st.sidebar.multiselect("Entry Start Min(s):", [0,15,30,45,50], default=esm_vals_opt_ui, key="esmvals_o12_fixed")
            if not esm_vals_opt_ui: esm_vals_opt_ui = [settings.DEFAULT_ENTRY_WINDOW_START_MINUTE]
            st.sidebar.markdown("**Entry End Hr Range:**"); c1,c2,c3=st.sidebar.columns(3); eeh_min_opt_ui=c1.number_input("Min Hr",value=eeh_min_opt_ui,min_value=0,max_value=23,step=1,key="eehmin_o12_fixed")
            eeh_max_opt_ui=c2.number_input("Max Hr",value=eeh_max_opt_ui,min_value=0,max_value=23,step=1,key="eehmax_o12_fixed")
            if opt_algo_ui=="Grid Search": eeh_steps_opt_ui=c3.number_input("Hr Steps",2,5,int(eeh_steps_opt_ui),1,key="eehsteps_o12_fixed")
        
        if opt_algo_ui=="Random Search": rand_iters_ui=st.sidebar.number_input("Random Iterations:",10,500,rand_iters_ui,10,key="randiter_o12_fixed")
    ...
    if analysis_mode_ui == "Walk-Forward Optimization":
        ...
        wfo_isd_ui = st.sidebar.number_input("In-Sample (Days):", min_value=MIN_WFO_IS_DAYS, value=st.session_state.get('wfo_isd_ui_val', settings.DEFAULT_WFO_IN_SAMPLE_DAYS), step=10, key="wfoisd_v12_fixed")
        wfo_oosd_ui = st.sidebar.number_input("Out-of-Sample (Days):", min_value=MIN_WFO_OOS_DAYS, value=st.session_state.get('wfo_oosd_ui_val', settings.DEFAULT_WFO_OUT_OF_SAMPLE_DAYS), step=5, key="wfoosd_v12_fixed")
        wfo_sd_ui = st.sidebar.number_input("Step (Days):", min_value=wfo_oosd_ui, value=st.session_state.get('wfo_sd_ui_val', settings.DEFAULT_WFO_STEP_DAYS), step=5, key="wfosd_v12_fixed")
    ...
    # --- Run Analysis Button ---
    if st.sidebar.button("Run Analysis", type="primary", use_container_width=True, key="run_main_v12_fixed"):
        ...
        if start_date_ui >= end_date_ui: st.error(f"Error: Start date ({start_date_ui}) must be before end date ({end_date_ui}).")
            ...
            if analysis_mode_ui == "Walk-Forward Optimization":
                ...
                if total_data_duration_days_for_check < min_required_wfo_duration:
                    ...
                    st.stop() # Stop execution for this specific error
            ...
            if price_data_df.empty: st.warning(f"No price data found for {selected_ticker_name} ({interval_for_this_run}) for the period {start_date_ui} to {end_date_ui}. Cannot proceed.")
                ...
                # Ensure AI service is available before trying to use it
                ...
                if (st.session_state.enable_ai_signal_filtering or st.session_state.enable_ai_regime_detection) and not current_ai_service:
                    st.error("AI features are enabled, but the AI Service is not available. Please check initialization. AI features will be skipped.")
                    # Optionally disable AI features for this run
                    # st.session_state.enable_ai_signal_filtering = False
                    # st.session_state.enable_ai_regime_detection = False
                
                strategy_params_for_engine = {
                    'strategy_name': selected_strategy_name,
                    'stop_loss_points': sl_points_single_ui,
                    'rrr': rrr_single_ui,
                    'ai_service': current_ai_service, # Pass AI service instance
                ...
                if analysis_mode_ui == "Single Backtest":
                    ...
                    if analysis_mode_ui == "Parameter Optimization":
                            ...
                            if not opt_df.empty:
                                ...
                                try:
                                    database_manager.save_optimization_results(
                                        ...
                                        results_df=opt_df,
                                        extra_config=db_opt_params_extra
                                    )
                                except Exception as db_save_err:
                                ...
                                if not valid_opt.empty:
                                    ...
                                    if selected_strategy_name == "Gap Guardian":
                                        ...
                                        best_ee_t = dt_time(int(best_r["EntryEndHour"]),int(best_r.get("EntryEndMinute",0))) # Use .get for safety
                        ...
                        wfo_optimizer_config_and_control = {
                            **optimizer_control_params, # This already contains AI service and flags, metric, strategy name, iterations
                        ...
                        with st.spinner(f"Running WFO with {opt_algo_ui}... This will take considerable time."):
                            wfo_log,oos_trades,oos_equity,oos_perf = optimizer.run_walk_forward_optimization(
                                ...
                                wfo_p, opt_algo_ui, wfo_optimizer_config_and_control, interval_for_this_run,
                                lambda p,s: opt_cb(p,s)
                            )
                            st.session_state.wfo_results = {"log":wfo_log,"oos_trades":oos_trades,"oos_equity_curve":oos_equity,"oos_performance":oos_perf} # Corrected key
    ...
    if not db_init_error and ai_service_available: # Only show if DB and AI service are fine
        st.sidebar.markdown("---")
                ...
                with st.spinner("Training signal filter model... (placeholder)"):
                    ...
                    if not signals_for_training.empty:
                        ...
                        if not features_df.empty:
                            ...
                            else: st.error("Could not align features and labels for training. Ensure signals and price data overlap.")
                        else: st.error("No features could be generated for training the AI signal filter.")
                    else: st.error("No signals available to generate features for AI model training. Run a backtest first or load historical signals.")
            elif not st.session_state.ai_service:
                st.warning("AI Service not available for training.")
            else: # Price data empty
                st.warning("Load price data first to enable AI model training (placeholder).")

    # --- Display Area ---
    main_tabs_to_display_names = []
    if st.session_state.get('backtest_results'): main_tabs_to_display_names.append("📊 Backtest Performance") # Use .get
    ...
    if main_tabs_to_display_names:
        # Ensure unique keys for tabs if content changes significantly based on run
        ...
        if "📊 Backtest Performance" in tab_map and st.session_state.backtest_results:
            with tab_map["📊 Backtest Performance"]:
                ...
                detail_tabs = st.tabs(detail_tabs_list) # Key will be auto-generated or use a dynamic one
                with detail_tabs[0]:
                ...
                with detail_tabs[3+idx_offset]:
                    if not st.session_state.price_data.empty:
                        ...
                        csv_data = st.session_state.price_data.to_csv(index=True).encode('utf-8'); st.download_button("Download Full Price Data CSV", csv_data, f"{ticker_symbol}_price_data.csv", 'text/csv', key='dl_raw_price_main_v12_fixed')
        ...
        if opt_tab_idx != -1 and not opt_results_df_to_check.empty:
            with tab_map["⚙️ Optimization Results (Full Period)"]:
                ...
                csv_opt_disp = opt_df_display.to_csv(index=False).encode('utf-8'); st.download_button("Download Optimization CSV", csv_opt_disp, f"{ticker_symbol}_{selected_strategy_name}_opt_results.csv", 'text/csv', key='dl_opt_csv_main_v12_fixed')
        ...
        if wfo_tab_idx != -1 and st.session_state.wfo_results:
            with tab_map["🚶 Walk-Forward Analysis"]:
                ...
                csv_wfo_log_disp = wfo_res_disp["log"].to_csv(index=False).encode('utf-8'); st.download_button("Download WFO Log CSV", csv_wfo_log_disp, f"{ticker_symbol}_{selected_strategy_name}_wfo_log.csv", 'text/csv', key='dl_wfo_log_main_v12_fixed')
                ...
                if not wfo_res_disp["oos_trades"].empty:
                    ...
                    csv_wfo_trades_disp = wfo_res_disp["oos_trades"].to_csv(index=False).encode('utf-8'); st.download_button("Download WFO OOS Trades CSV", csv_wfo_trades_disp, f"{ticker_symbol}_{selected_strategy_name}_wfo_oos_trades.csv", 'text/csv', key='dl_wfo_trades_main_v12_fixed')
                else: st.info("No out-of-sample trades generated during WFO.")
    
    elif st.session_state.get('run_analysis_clicked_count',0) > 0 : # Use .get
    ...
    elif not db_init_error: # Only show initial message if DB is fine
        st.info("Configure parameters in the sidebar and click 'Run Analysis'.")
    # If db_init_error is True, the critical DB error message is already shown at the top.

    # --- Footer / Version Info ---
    st.sidebar.markdown("---")
    st.sidebar.info(f"App Version: 0.7.1 | AI Integrated | Last Updated: {datetime.now().strftime('%Y-%m-%d %H:%M')}")
...
else: # This else corresponds to "if not db_init_error:" for the main UI block
    st.warning("Application UI cannot be fully displayed due to the critical database error mentioned above.")
    # You could offer some very basic functionality here if needed, or just stop.
